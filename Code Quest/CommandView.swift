...
class CommandView: UIView {
	...
	required init?(coder aDecoder: NSCoder) {
		...
		for i in 0..<num_command_buttons {
			...
			let command = Input(type: ButtonType(rawValue : i)!, frame: CGRect(x: ViewController.scaleDims(input: xcoord, x: true), y: 0, width: ViewController.scaleDims(input: 96, x: true), height: ViewController.scaleDims(input: 96, x: false)));
